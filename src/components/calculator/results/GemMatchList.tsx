...
export function GemMatchList({
  ...
  return (
    <div className={className}>
      {label && (
        <h3 className="text-sm font-medium text-slate-700 dark:text-cream-secondary mb-3">
          {label}
          {gems.length > 0 && (
            <span className="ml-2 text-slate-600 dark:text-cream-muted font-normal">
      ...
      {gems.length > 0 ? (
        ...
        <p className="text-sm text-slate-600 dark:text-cream-secondary py-4 text-center bg-slate-50 dark:bg-coffee-raised2 rounded-lg">
          {emptyMessage}
        </p>
      )}
    </div>
